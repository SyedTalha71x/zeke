...
export const createCardPack = async (req, res) => {
  try {
    ...
    const newCardPack = new CardPack({
      ...
      tier1CardsCount: Number(tier1CardsCount) || 0,
      tier2CardsCount: Number(tier2CardsCount) || 0
    });
...
export const updateCardPack = async (req, res) => {
  try {
    ...
    if (tier1CardsCount !== undefined) updatedCard.tier1CardsCount = Number(tier1CardsCount);
    if (tier2CardsCount !== undefined) updatedCard.tier2CardsCount = Number(tier2CardsCount);
...
export const deleteCardPack = async (req, res) => {
  try {
    ...
    await Card.deleteMany({ cardPack: cardPack._id });
    
    await CardPack.findByIdAndDelete(req.params.id);

    res.status(200).json({
      success: true,
      message: "Card pack and all associated cards deleted successfully",
    });
  } catch (error) {
